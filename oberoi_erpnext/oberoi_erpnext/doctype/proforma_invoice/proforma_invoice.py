...
class ProformaInvoice(SellingController):
	...
	def validate_sales_mntc_sales_order(self):
		for d in self.get('items'):
			if d.prevdoc_docname:
				res = frappe.db.sql("select name from `tabSales Order` where name=%s and order_type = %s",
					(d.prevdoc_docname, self.order_type))
				if not res:
					frappe.msgprint(_("Sales Order {0} not of type {1}")
						.format(d.prevdoc_docname, self.order_type))

	def validate_order_type(self):
		super(ProformaInvoice, self).validate_order_type()
	...
	def validate_delivery_date(self):
		...
		self.validate_sales_mntc_sales_order()
	...
	def validate_warehouse(self):
		super(ProformaInvoice, self).validate_warehouse()
	...
	def validate_with_previous_doc(self):
		super(ProformaInvoice, self).validate_with_previous_doc({
			"Sales Order": {
	...
	def update_prevdoc_status(self, flag):
		for sales_order in list(set([d.prevdoc_docname for d in self.get("items")])):
			if sales_order:
